...
type Shift = {
  id: string; work_date: string; hours: number; departments: Dept[];
  comment: string | null; rates_snapshot: Record<string, number>;
};
type Settings = {
...
export default function DashboardClient({
  initialShifts,initialSettings,initialProfile,email
}:{
  initialShifts:Shift[];initialSettings:Settings|null;initialProfile:Profile|null;email:string
}){
  ...
  const [hours,setHours]=useState(""); const [comment,setComment]=useState("");
  const [departments,setDepartments]=useState<Dept[]>([]); const [status,setStatus]=useState("");

  function calc(s:Shift|{hours:number;departments:Dept[];rates_snapshot?:Record<string,number>}){
    const snap=s.rates_snapshot||{P01:settings.p01,P02:settings.p02,P03:settings.p03,P21:settings.p21,P28:settings.p28,hourly_rate:settings.hourly_rate,housing_bonus:settings.housing_bonus};
  ...
  async function saveShift(){
    ...
    const snapshot={P01:settings.p01,P02:settings.p02,P03:settings.p03,P21:settings.p21,P28:settings.p28,hourly_rate:settings.hourly_rate,housing_bonus:settings.housing_bonus};
    const {data,error}=await supabase.from("shifts").insert({work_date:date,hours:Number(hours||0),departments,comment:comment||null,rates_snapshot:snapshot}).select().single();
    if(error)return setStatus(error.message);
    setShifts([data,...shifts]);setDepartments([]);setHours("");setComment("");setShowForm(false);
  }
  ...
  async function saveSettings(){
    ...
    setStatus(error?error.message:"Личные настройки сохранены.");
  }

  async function saveProfile(){
    const name=displayName.trim(); if(!name)return setStatus("Укажи имя.");
    const {error}=await supabase.from("profiles").upsert({display_name:name},{onConflict:"user_id"});
    setStatus(error?error.message:"Профиль сохранён.");
  }

  async function shareSite(){
    const url=window.location.origin;
    try{await navigator.clipboard.writeText(url);setStatus("Ссылка на сайт скопирована.");}
    catch{setStatus(url);}
  }
  ...
  return <main className="app-shell">
    <header className="topbar">
      <div><div className="brand"><span className="brand-mark">B</span><span>Biedronka HAKY Salary</span></div><div className="muted">Пользователь: {displayName||email}</div></div>
      <button className="secondary" onClick={signOut}>Выйти</button>
    </header>

    <section className="hero">
      <div className="eyebrow">Личный кабинет · данные видишь только ты</div>
    ...
    <div className="action-row">
      ...
      <button className="secondary share-btn" onClick={shareSite}>Поделиться сайтом</button>
    </div>
    ...
    {showForm&&<section className="card form">
      ...
      {departments.map((d,i)=><div className="dept-row" key={`${d.department}-${i}`}>
        <select value={d.department} onChange={e=>{const copy=[...departments];copy[i]={...copy[i],department:e.target.value};setDepartments(copy)}}>
          {["P01","P02","P03","P21","P28"].map(x=><option key={x} value={x} disabled={departments.some((z,j)=>j!==i&&z.department===x)}>{x}</option>)}
        </select>
        <input type="number" placeholder="Картоны" value={d.cartons||""} onChange={e=>{const copy=[...departments];copy[i]={...copy[i],cartons:Number(e.target.value)};setDepartments(copy)}}/>
    ...
    <section><div className="section-title"><h2>Последние смены</h2></div><div className="list">
    ...
    <section><div className="section-title"><h2>Мой профиль</h2></div><div className="card settings-grid">
      <label>Имя<input value={displayName} onChange={e=>setDisplayName(e.target.value)}/></label>
      <label>Email<input value={email} disabled/></label>
      <button className="primary" onClick={saveProfile}>Сохранить профиль</button>
    </div></section>

    <section><div className="section-title"><h2>Мои ставки</h2></div><div className="card settings-grid">
      {(["P01","P02","P03","P21","P28"] as const).map(k=><label key={k}>{k}<input type="number" step=".0001" value={settings[keyMap[k]] as number} onChange={e=>setSettings({...settings,[keyMap[k]]:Number(e.target.value)})}/></label>)}
      ...
      <button className="primary" onClick={saveSettings}>Сохранить мои настройки</button>
    </div></section>
  </main>;
}
