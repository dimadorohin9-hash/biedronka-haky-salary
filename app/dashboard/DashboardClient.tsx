...
const defaults:Settings={p01:0,p02:0,p03:0,p21:0,p28:0,hourly_rate:0,housing_bonus:0,salary_goal:7000};
...
export default function DashboardClient({initialShifts,initialSettings,initialProfile,email}:{initialShifts:Shift[];initialSettings:Settings|null;initialProfile:Profile|null;email:string}){
 ...
 function exportCsv(){
  const rows=[["Р”Р°С‚Р°","Р§Р°СЃС‹","РћС‚РґРµР»С‹","РљР°СЂС‚РѕРЅС‹","РљР°СЂС‚РѕРЅС‹ PLN","РџРѕС‡Р°СЃРѕРІР°СЏ PLN","Р–РёР»СЊС‘ PLN","РС‚РѕРіРѕ PLN","РљРѕРјРјРµРЅС‚Р°СЂРёР№"]];
  monthShifts.slice().reverse().forEach(s=>{
   const c=calc(s);
   rows.push([
    s.work_date,
    String(s.hours||0),
    s.departments.map(d=>d.department).join("+")||"РўРѕР»СЊРєРѕ С‡Р°СЃС‹",
  ...
  setStatus("Р¤Р°Р№Р» РґР»СЏ Excel СЌРєСЃРїРѕСЂС‚РёСЂРѕРІР°РЅ.");
 }
 ...
 return <main className="app-shell">
  <header className="topbar"><div><div className="brand"><span className="brand-mark">B</span><span>Biedronka Salary</span></div><div className="muted">РџРѕР»СЊР·РѕРІР°С‚РµР»СЊ: {displayName||email}</div></div><button className="secondary compact" onClick={signOut}>Р’С‹Р№С‚Рё</button></header>
  ...
  {tab==="stats"&&<><div className="section-title"><h2>РЎС‚Р°С‚РёСЃС‚РёРєР°</h2></div><section className="grid"><div className="card"><div className="stat-label">Р—Р°СЂР°Р±РѕС‚Р°РЅРѕ</div><div className="stat-value">{money(totals.total)}</div></div><div className="card"><div className="stat-label">РЎСЂРµРґРЅСЏСЏ СЃРјРµРЅР°</div><div className="stat-value">{money(monthShifts.length?totals.total/monthShifts.length:0)}</div></div><div className="card"><div className="stat-label">Р›СѓС‡С€РёР№ РґРµРЅСЊ</div><div className="stat-value small-value">{bestDay?dateText(bestDay[0]):"вЂ”"}</div><div className="muted">{bestDay?money(bestDay[1]):""}</div></div><div className="card"><div className="stat-label">Р”Рѕ С†РµР»Рё</div><div className="stat-value">{money(Math.max(0,settings.salary_goal-totals.total))}</div></div></section><div className="section-title"><h2>РћС‚РґРµР»С‹</h2></div><div className="card dept-stats">{deptStats.map(([d,v])=><div className="dept-stat" key={d}><div><strong>{d}</strong><span>{v.cartons.toLocaleString("pl-PL")} РєР°СЂС‚РѕРЅРѕРІ</span></div><strong>{money(v.pay)}</strong></div>)}</div><div className="section-title"><h2>Р­РєСЃРїРѕСЂС‚</h2></div><div className="action-row export-actions"><button className="primary" onClick={exportCsv}>Р­РєСЃРїРѕСЂС‚ РІ Excel</button><button className="secondary" onClick={printPdf}>РџРµС‡Р°С‚СЊ / PDF</button></div><div className="section-title"><h2>РђСЂС…РёРІ Р·Р°СЂРїР»Р°С‚</h2></div><div className="card archive-list">{archive.map(([k,v])=><button className="archive-row" key={k} onClick={()=>{setMonth(k);setTab("home");}}><div><strong>{monthTitle(k)}</strong><span>{v.shifts} СЃРјРµРЅ В· {v.hours} С‡. В· {v.cartons.toLocaleString("pl-PL")} РєР°СЂС‚РѕРЅРѕРІ</span></div><strong>{money(v.total)}</strong></button>)}{!archive.length&&<div className="empty-box">РђСЂС…РёРІ РїРѕСЏРІРёС‚СЃСЏ РїРѕСЃР»Рµ РїРµСЂРІРѕР№ СЃРјРµРЅС‹.</div>}</div>{status&&<p className="notice">{status}</p>}</>}
